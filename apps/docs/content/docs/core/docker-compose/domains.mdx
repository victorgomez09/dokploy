...
1. Add the service to the `alvi-paas-network`.
...
First, we'll add the alvi-paas-network to our services:
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.alvi-paas.com')`
   This label tells Traefik that the domain to be used is `your-domain.alvi-paas.com`
...
services:
  frontend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.frontend-app.rule=Host(`frontend.alvi-paas.com`)
  ...
  backend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.backend-app.rule=Host(`backend.alvi-paas.com`)
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.alvi-paas.com')` Specifies the domain for the service
