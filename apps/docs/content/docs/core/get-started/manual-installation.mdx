...
description: "Learn how to manually install Alvi Paas on your server."
---

If you wish to customize the Alvi Paas installation on your server, you can modify several aspects:

1. **Use a specific port for Alvi Paas** - Ideal for avoiding conflicts with other services.
2. **Select a specific version of Alvi Paas** - Choose between versions like canary, latest, or a specific Docker tag (e.g., v0.0.1).
3. **Specify HTTP and HTTPS ports for Traefik** - Useful when running alongside other servers like Nginx.

## Installation Script

Here is a Bash script for installing Alvi Paas on a Linux server. Make sure you run this as root on a Linux environment that is not a container, and ensure ports 80 and 443 are free.
...
# Create network
docker network rm -f alvi-paas-network 2>/dev/null
docker network create --driver overlay --attachable alvi-paas-network
echo "Network created"

# Prepare configuration directory
mkdir -p /etc/alvi-paas
chmod -R 777 /etc/alvi-paas

# Pull and deploy Alvi Paas
docker pull alvi-paas/alvi-paas:latest
docker service create \
  --name alvi-paas \
  --replicas 1 \
  --network alvi-paas-network \
  --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock \
  --mount type=bind,source=/etc/alvi-paas,target=/etc/alvi-paas \
...
BLUE="\033[0;34m"
NC="\033[0m" # No Color
printf "${GREEN}Congratulations, Alvi Paas is installed!${NC}\n"
printf "${BLUE}Wait 15 seconds for the server to start${NC}\n"
printf "${YELLOW}Please go to http://${advertise_addr}:3000${NC}\n\n"

```

This script includes checks for common pitfalls, installs Docker if it’s not already installed, initializes a Docker Swarm, creates a network, and then pulls and deploys Alvi Paas. After the script runs, it provides a success message and instructions for accessing Alvi Paas.
...
If you already have a Docker swarm running on your server and you want to use alvi-paas, you can use the following command to join it:
