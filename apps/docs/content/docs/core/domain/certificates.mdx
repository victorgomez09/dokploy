---
title: "Certificates"
description: "Learn how to set up certificates for the Alvi Paas panel"
---

import { Callout } from "fumadocs-ui/components/callout";

When using a domain for Alvi Paas, we offer 3 options for certificates:
...
<Callout title="Cloudflare">
  When you register a domain on Cloudflare, the SSL certificate is automatically
  assigned by Cloudflare, so you need to select `None` in Alvi Paas and disable
  HTTPS.
</Callout>
...
1. In Alvi Paas, select `Let's Encrypt` and enable `HTTPS`.
...
2. In Alvi Paas, select `Let's Encrypt` and enable `HTTPS`.
...
<Callout>
  **Note:** When creating a domain in your application, ensure you use the
  `Let's Encrypt` certificate and enable `HTTPS`. The generation of certificates
  typically takes about 20 seconds. If the certificate is not generated, restart
  Traefik and try again.
</Callout>
