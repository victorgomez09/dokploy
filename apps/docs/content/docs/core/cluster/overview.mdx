...
description: "Learn how to set up and manage a cluster in Alvi Paas with docker swarm."
sidebar:
  order: 1
---

For most suitables cases you may not need to use a cluster, unless you need to scale your application.
A cluster is a group of nodes that work together to provide a single, unified service. In Alvi Paas, clusters manage a group of servers collaborating to deliver this unified service.
...
- A server with Alvi Paas installed.
...
4. Follow the two steps provided:
   ...
   - Install the Alvi Paas agent. These steps are the same for both node types.
...
/>
<ImageZoom
  src="cluster/application.png"
  width={800}
  height={630}
  alt="home og image"
  className="rounded-lg"
/>
{/* ![Add Node](cluster/nodes.png)
![Cluster](cluster/cluster.png)
![Application Setting](cluster/application.png) */}
...
2. Fork the repository [Alvi Paas/swarm-test](https://github.com/Alvi Paas/swarm-test).
...
To add manager nodes to the cluster, follow the same steps as adding a worker node. However, if the manager node (where Alvi Paas is running) fails, all other nodes will become inaccessible. This is because we currently have a single entry point for the cluster. To ensure high availability, you should have multiple manager nodes.
...
<ImageZoom
  src="cluster/swarm-settings.png"
  width={800}
  height={630}
  alt="home og image"
  className="rounded-lg"
