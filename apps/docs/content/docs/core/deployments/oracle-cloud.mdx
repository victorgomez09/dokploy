...
<ImageZoom
  src="deployment/oracle/oracle-shape.png"
  width={800}
  height={630}
  alt="home og image"
...
6. Login to the instance `ssh ubuntu@ip-address-of-instance`
   <Callout>
     If you cannot access to the instance because you se connection refused
     error, you can try adding a `Public Subnet IPv4 CIDR Block` to the
     instance, go to your instance and then Quick Actions -> Connect public
     subnet to internet -> Create
   </Callout>
7. Run `sudo su`
8. Run `curl -sSL https://alvi-paas.com/install.sh | sh`
...
10. Go to `ip-address-of-instance:3000` and you will see the dashboard.
